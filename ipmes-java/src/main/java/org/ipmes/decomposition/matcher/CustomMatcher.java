 ...
 */
public class CustomMatcher implements TCMatcher {
    ...
    Join join;
    int poolSize;
    public CustomMatcher(Collection<TCQuery> tcQueries, boolean useRegex, long windowSize, Join join) {
    ...
    public void sendAll(ArrayList<EventEdge> events) {
        if (events.isEmpty())
            return;
