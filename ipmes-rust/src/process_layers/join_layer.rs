    ...
    fn generate_relations(
        ...
        for &edge1 in sub_patterns[self.id].edges {
            for &edge2 in sub_patterns[self.sibling_id].edges {
                ...
                let distance_1_2 = distances_table.get(&(id1, id2))?;
                let distance_2_1 = distances_table.get(&(id2, id1))?;
...
impl<P> JoinLayer<P> {
    pub fn new(prev_layer: P, pattern: &Pattern, sub_patterns: &Vec<SubPattern>) -> Self {
        Self {
            prev_layer,
            num_sub_patterns: sub_patterns.len(),
            sub_pattern_buffers: vec![BinaryHeap::new(), 2 * sub_patterns.len() - 1],
