...
fn test_try_merge_nodes1() {
    ...
    let mut used = vec![false; max_node_id as usize];
    let merged = try_merge_nodes(&a, &b, &mut used);
...
fn test_try_merge_edges1() {
    ...
    /// "timestamp" is useless here.
    let mut timestamps = vec![0, 20];
    assert!(try_merge_match_edges(&match_edge1, &match_edge2, &mut timestamps).is_none());
}
...
fn test_try_merge_edges2() {
    ...
    let res = try_merge_match_edges(&match_edge1, &match_edge2, &mut timestamps);
    assert!(!res.is_none());
...
fn test_try_merge_nodes_large(){
    ...
    if let Some(merged) = try_merge_nodes(&a, &b, &mut used) {
        assert_ne!(ans, None);
